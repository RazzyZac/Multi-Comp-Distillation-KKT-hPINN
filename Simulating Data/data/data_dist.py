...
class StreamDataGeneratorAspenDIST:
    ...
    def _get_flow_data(self):
        ...
        for block in self.all_blocks:
            ...
            self.stream_dict[block]["#-TRAYS"] = 0
        ...
        for block in self.all_blocks:
            ...
            #number of trays
            num_tray_path = "\\Data\\Blocks\\{}\\Input\\NSTAGE".format(block)
            num_tray_value = self.aspen.Tree.FindNode(num_tray_path).Value
            if num_tray_value is not None:
                self.stream_dict[block]["#-TRAYS"] = num_tray_value
            else:
                self.stream_dict[block]["#-TRAYS"] = 'Returned None' 

            #feed tray position
            feed_tray_path = "\\Data\\Blocks\\{}\\Input\\FEED_STAGE\\FEED".format(block)
            feed_tray_value = self.aspen.Tree.FindNode(feed_tray_path).Value
            if feed_tray_value is not None:
                self.stream_dict[block]["FEED_TRAY"] = feed_tray_value
            else:
                self.stream_dict[block]["FEED_TRAY"] = 'Returned None' 
            ...
            #Pressure
            pressure_path = "\\Data\\Blocks\\B1\\Input\\PRES1"
            pressure_value = self.aspen.Tree.FindNode(pressure_path).Value
            if pressure_value is not None:
                self.stream_dict[block]["PRES"] = pressure_value
            else:
                self.stream_dict[block]["PRES"] = 'Returned None'
